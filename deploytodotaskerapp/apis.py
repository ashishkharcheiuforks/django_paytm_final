...
from deploytodotaskerapp.serializers import (
...
def customer_add_order(request):
    """
        params:
            ...
            order_details (json format), example:
                [{"meal_id": 1, "drink_id": 1, "quantity": 2},{"meal_id": 2, "drink_id": 2, "quantity": 3}]
            stripe_token
    ...
    if request.method == "POST":
        ...
        for meal in order_details:
            order_total += (Meal.objects.get(id=meal["meal_id"]).price * meal["quantity"] + Drink.objects.get(id=drink["drink_id"]).price * drink["quantity"])
            )
...
def response(request):
    ...
    if request.method == "POST":
        ...
        back_response = {"PAY_STATUS": status}
        ...
        if status == "TXN_SUCCESS":
            ...
            for meal in order_details:
                OrderDetails.objects.create(
                    ...
                    + (
                        Drink.objects.get(id=drink["drink_id"]).price
                        * drink["quantity"]
                    ),
                )
    ...
    if request.method == "POST":
        ...
        try:
            order = Order.objects.get(
                id=request.POST["order_id"], driver=None, status=Order.READY
            )
            order.driver = driver
